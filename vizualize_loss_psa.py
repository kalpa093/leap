...
from viz_utils import save_loss_landscape
...
def train_model(model_key, train_df, val_df, test_df, output_file, is_multilabel=False, dataset_key="App", da_key="none"):
    if is_multilabel:
        label_list = sorted(
            set(
                ",".join(
    ...
    viz_dir = f"viz_results/{dataset_key}_{da_key}_{model_key}"
    os.makedirs(viz_dir, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    save_loss_landscape(trainer.model, trainer.get_eval_dataloader(), device, f"{viz_dir}/landscape.pdf")
...
def main():
    ...
    for i in range(args.iteration):
        ...
        if args.da:
            aug_path = f"temp/{args.dataset}_{args.da}.csv"
            print(f"Loading augmented dataset from: {aug_path}")
            if os.path.exists(aug_path):
                train_df = pd.read_csv(aug_path)
                train_df = train_df.dropna(subset=['text', 'label'])
                train_df['text'] = train_df['text'].astype(str)
                print(f"Successfully loaded {len(train_df)} samples.")
            else:
                raise FileNotFoundError(f"file not found at: {aug_path}")
        else:
            train_df = apply_da_script(args.dataset, args.da, train_df)
        train_model(args.model, train_df, val_df, test_df, output_file, is_multilabel, args.dataset, args.da)
